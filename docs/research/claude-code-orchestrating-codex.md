...
- **Nested bubblewrap fails in unprivileged containers.** "In an unprivileged container, bubblewrap cannot mount a fresh `/proc` filesystem. Set `enableWeakerNestedSandbox` to `true`…" ([sandboxing § Troubleshooting](https://code.claude.com/docs/en/sandboxing#troubleshooting)) The same class of failure can bite a Codex Landlock sandbox launched inside a bubblewrap sandbox. **Unverified**: whether Codex's Landlock sandbox actually initializes correctly inside Claude Code's bubblewrap sandbox.
  > **Measured since — both halves of this were wrong** ([probe](./sandbox-nesting-probe.md)). Codex's Linux sandbox is not Landlock in practice: it shells out to its own **bubblewrap** helper, which needs a writable `/tmp` to build synthetic mount targets and panics without one. And `enableWeakerNestedSandbox` fixes nothing here — it changes `/proc` visibility and capabilities, neither of which is what the helper is missing. Granting `/tmp` is what makes nesting work.
...
1. **Does Codex's Landlock/Seatbelt sandbox initialize correctly inside Claude Code's bubblewrap/Seatbelt sandbox?** Both docs describe nesting hazards generically ([sandboxing § Troubleshooting](https://code.claude.com/docs/en/sandboxing#troubleshooting) mentions `enableWeakerNestedSandbox` for bubblewrap-in-container), but neither documents this specific pairing. Needs empirical testing on Linux and macOS.
   > **Answered on Linux; macOS still open** ([probe](./sandbox-nesting-probe.md), [ADR-0004](../adr/0004-disable-codex-sandbox-under-outer-sandbox.md)). Yes, it initialises — provided `/tmp` and `$CODEX_HOME` are writable. Codex's Linux sandbox is bubblewrap-based, not Landlock, and its helper builds mount targets under `/tmp`; with `/tmp` read-only it panics and reports the crash as a denial with exit code `0`. `enableWeakerNestedSandbox` is irrelevant either way. macOS is untested and the Linux cause does not transfer to Seatbelt.
